...
*** Renewing subkeys
    ...
    At the time of writing, DrDuh's guide doesn't go into much detail
    on how to renew your key's subkeys. [[https://unix.stackexchange.com/a/177310][This StackExchange answer]]
    shows you how to do this with GnuPG. Once that's finished, you'll
    need to re-export your keys and move them to your YubiKey(s)
    again, of course. You'll also need to re-enable per-use touch
    (e.g., ~ykman openpgp set-touch sig on~), as updating the keys
    resets this setting to ~off~ for the corresponding key.
